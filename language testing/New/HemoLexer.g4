lexer grammar HemoLexer;

channels { BYTE_ORDER_MARK, WHITESPACE, COMMENT, DIRECTIVE }

Utf8ByteOrderMark: '\u00EF\u00BB\u00BF' -> channel(BYTE_ORDER_MARK);

EndOfFile: EOF;

SingleLineDocComment: '///' InputCharacter*   -> channel(COMMENT);
DelimitedDocComment:  '/**' .*? '*/'          -> channel(COMMENT);
SingleLineComment:    '//'  InputCharacter*   -> channel(COMMENT);
DelimitedComment:     '/*'  .*? '*/'          -> channel(COMMENT);
...
HexadecimalIntegerLiteral
	:	Sign? HexPrefix HexNumeral
	;

OctalIntegerLiteral
	:	Sign? OctalPrefix OctalNumeral
	;

BinaryIntegerLiteral
	:	BinaryPrefix BinaryNumeral
	;

DecimalRealLiteral
	: Sign? DecimalNumeral '.' DecimalNumeral 'e' Sign? DecimalNumeral?
	| '.' DecimalNumeral 'e' Sign? DecimalNumeral?
	| DecimalNumeral 'e' Sign? DecimalNumeral?
	;
...
DecimalNumeral
	:	'0'
	|	NonZeroDigit (Digits? | DigitSeparators Digits)
	;
...
fragment
Digits
	:	Digit (DigitsAndDigitSeparators? Digit)?
	;
...
fragment
DigitsAndDigitSeparators
	:	DigitOrDigitSeparator+
	;
...
fragment
HexDigits
	:	HexDigit (HexDigitsAndDigitSeparators? HexDigit)?
	;
...
fragment
HexDigitsAndDigitSeparators
	:	HexDigitOrDigitSeparator+
	;
...
OctalNumeral
	:	DigitSeparators? OctalDigits
	;

fragment
OctalDigits
	:	OctalDigit (OctalDigitsAndDigitSeparators? OctalDigit)?
	;
...
fragment
OctalDigitsAndDigitSeparators
	:	OctalDigitOrDigitSeparator+
	;
...
fragment
BinaryDigits
	:	BinaryDigit (BinaryDigitsAndDigitSeparators? BinaryDigit)?
	;
...
fragment
BinaryDigitsAndDigitSeparators
	:	BinaryDigitOrDigitSeparator+
	;
...
StorageSize : /* 1 to 128 */ '1'..'9' | ('1'..'9' '0'..'9') | ('1' '0'..'1' '0'..'9') | ('1' '0'..'2' '0'..'8');
