...
INTERPOLATED_REGULAR_STRING_START:   '$"'
    { interpolatedStringLevel++; interpolatedVerbatums.Push(false); verbatum = false; } -> pushMode(INTERPOLATION_STRING);
INTERPOLATED_VERBATUM_STRING_START: '$@"'
    { interpolatedStringLevel++; interpolatedVerbatums.Push(true); verbatum = true; }  -> pushMode(INTERPOLATION_STRING);
...
REGULAR_CHAR_INSIDE:           { !verbatum }? SimpleEscapeSequence;
VERBATUM_DOUBLE_QUOTE_INSIDE: {  verbatum }? '""';
DOUBLE_QUOTE_INSIDE:           '"' { interpolatedStringLevel--; interpolatedVerbatums.Pop();
    verbatum = (interpolatedVerbatums.Count > 0 ? interpolatedVerbatums.Peek() : false); } -> popMode;
REGULAR_STRING_INSIDE:         { !verbatum }? ~('{' | '\\' | '"')+;
VERBATUM_INSIDE_STRING:       {  verbatum }? ~('{' | '"')+;
